...
var svg_height = 1.5 * window.innerHeight;
...
d3.queue()
        ...
        .await(function(error, vocab) {
             allWords = vocab.map(d => d.word)

var testArray =  allWords.slice(1,4000)

var concretewords = ["bipolar", "christianity","communism","epiphany","flip","trump","grinnell","hipster","home","homophobia","isis","islam","korea","maddie","microsoft","obama","queer","racism","sonder","sonorous","syria","terrorism","transcend","uber"];

for(var word of concretewords){
    testArray.push(word);
}
testArray.sort();
...
function createSVG(searchword) { 
    d3.queue()
        .defer(d3.json, "/model/" + searchword)
...
*/
    ...
    //Step 4: Set up link and node
     link = svg.select("g")
      ...
      node = svg.select("g")
                ...
                .attr("r", d => sizeScale(layerMap.get(d.word)))
    ...
    .distance(d => distanceScale(1-d.value));
...
optArray = optArray.sort();
     ...
     var testArray = allWords.slice(1,5);
...
function searchWord() {
    ...
    searchword = String(document.getElementById('wordsearch').value);
    console.log(searchword);

    createSVG(searchword);
...
function exit_highlight(){
    ...
    //*************** For sliders
$("#num_subnodes").on("slide", function(slideEvt) {
    console.log("neighbor slider update" + NUM_NEIGHBOR);
